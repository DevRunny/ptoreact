      ...
      <div className={style.background}>
        ...
        <form className={style.sendForm} onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {/* <div className={style.formItem}>
            <label htmlFor="">Выберите категорию ТС:</label>
            <select
              className={style.select}
              value={form.category}
              onChange={(e) => {
                form.onChangeCategory(e);
              }}
            >
              {form.categories.map((cat) => {
                return (
                  <option className={style.option} value={cat.categoryName}>
                    {cat.categoryName} - категория{" "}
                  </option>
                );
              })}
            </select>
            <p className={style.descriptionCategory}>
              {form.currentCategoryDesc}
            </p>
          </div> */}
